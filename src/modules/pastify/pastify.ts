...
export function splitMessage(message: string, threshold: number = config.pastebin.threshold) {
 ...
 * @param force whether to pastify the message regardless of whether there's any code
...
(message: Message, force?: force, threshold?: number):
	Promise<PastifyReturn<force>> {
	const split = splitMessage(message.content, threshold)

	// if it's just a string, do nothing
	if (!force && !split.some(part => 'content' in part)) return null as PastifyReturn<force>
