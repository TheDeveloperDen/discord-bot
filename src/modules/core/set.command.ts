...
export const SetCommand: Command<ApplicationCommandType.ChatInput> = {
    ...
    async handle(interaction: CommandInteraction) {
        ...
        if ((channel == null) || channel instanceof StageChannel) {
            await interaction.reply('You can\'t do this in DMs')
            return
        }
        if (channel.partial || channel instanceof PartialGroupDMChannel) {
            await interaction.reply('Channel is partial')
            return
        }
        ...
        >(
            {
                filter: (i) =>
                    i.isButton() && i.message?.interactionMetadata?.id ===
                    interaction.id &&
